...
# old includes 
INCLUDES="tcpdump,iptables,ssh,libxml2,portmap,nfs-common,screen,sudo,libsqlite3-0,iperf,gdb,libdbus-1-3,libbluetooth2";
...
if [ "`grep "Debian" /etc/issue`" != "" ]; then
	...
	if ! debootstrap --verbose --arch amd64 --include=$INCLUDES $DEBIAN_VERSION $MOUNT_PATH http://ftp.se.debian.org/debian; then
...
echo 'disk = [ "file:'$IMAGE_PATH'/$NODENAME.img,sda1,w", "file:'$SWAP_PATH'/$NODENAME-swap.img,sda2,w" ]' >> $CONFIG_PATH/node-xencfg.template
