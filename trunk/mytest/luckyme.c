...
#define ATTRIBUTE_NAME "datainfo"
#define ATTRIBUTE_VALUE "haggletestbed"
...
//send contacts
struct dataobject* haggle_create_dataobject_file(char *file_name)
{
	...
	LIBHAGGLE_DBG("DataObject Created (%s)\n", strTime);
...
//the main part to run this application
void test_loop() {
	int result = 0;
...
//#endif
		...
		sleep(20);
		//haggle_publish_contacts_info(hh,dirpath);
		
		haggle_create_dataobject_file(dirpath);
	}
